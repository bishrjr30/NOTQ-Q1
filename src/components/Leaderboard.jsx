...
export default function Leaderboard() {
  ...
  const loadLeaderboard = async () => {
    try {
      // نجلب الطلاب من جدول students في Supabase
      const { data, error } = await supabase
        .from("students")
        .select("id, name, level, total_exercises, show_on_leaderboard")
      ...
      // نفس منطق الترتيب القديم
      const rankedStudents = allStudents
        .filter((s) => s.show_on_leaderboard !== false) // الافتراضي: يظهر في اللوحة إذا لم يكن false
        .sort(
          (a, b) =>
            ((b.total_exercises || 0) * 10) -
            ((a.total_exercises || 0) * 10)
        )
  ...
  if (loading)
    return (
      <div className="text-center p-4 arabic-text">
  ...
  return (
    <Card className="border-0 shadow-2xl bg-gradient-to-br from-yellow-50 to-orange-50 overflow-hidden relative">
      ...
      <CardHeader className="text-center relative z-10">
        ...
        <CardTitle className="text-2xl font-bold text-orange-900 arabic-text">
          لوحة الصدارة 🏆
        </CardTitle>
        <p className="text-orange-700 text-sm arabic-text">
          أبطال القراءة لهذا الأسبوع
        </p>
      </CardHeader>
      ...
      <CardContent className="relative z-10 px-4 pb-6">
        <div className="space-y-3">
          {students.map((student, index) => (
            <motion.div
              ...
              className={`flex items-center justify-between p-3 rounded-xl border-2 transition-all ${
                index === 0
                  ? "bg-yellow-100 border-yellow-300 shadow-md scale-105 my-4"
                  : index === 1
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-800 arabic-text text-sm md:text-base">
                    {student.name}
                    {index === 0 && (
                      <Crown className="w-4 h-4 text-yellow-600 inline mr-1" />
                    )}
                  </p>
                  <div className="flex items-center gap-1">
                    <Badge
                      variant="secondary"
                      className="text-[10px] h-5 px-1 bg-white/50"
                    >
                      {student.level}
                    </Badge>
                  </div>
          ...
          {students.length === 0 && (
            <p className="text-center text-gray-500 arabic-text py-4">
              لا يوجد متصدرين بعد.. كن الأول!
            </p>
          )}
        </div>
      </CardContent>
