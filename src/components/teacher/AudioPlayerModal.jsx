...
export default function AudioPlayerModal({
  ...
  React.useEffect(() => {
    if (isOpen) {
      ...
      // لو الرابط أصلاً غير موجود أو غير صالح
      if (!audioUrl || audioUrl.trim() === "") {
        setIsLoading(false);
        setError(true);
        setErrorMessage("رابط الملف الصوتي غير متوفر.");
      }
    }
  }, [isOpen, audioUrl]);
        ...
        <div className="py-4 space-y-4">
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-sm text-gray-600 arabic-text">
              ...
              {audioUrl ? `${audioUrl.substring(0, 60)}...` : "غير متوفر"}
            </p>
          </div>
          ...
          {!error && audioUrl && (
            <div className="space-y-3">
              ...
              >
                {/* Supabase public URL عادةً يكون بصيغة https ويمكن تشغيله كأي ملف عادي */}
          ...
          <details className="mt-4">
            <summary className="text-xs text-gray-500 cursor-pointer arabic-text">
              معلومات تقنية (للتشخيص)
            </summary>
            <div className="mt-2 p-2 bg-gray-50 rounded text-xs text-gray-600 space-y-1">
              <p>
                <strong>رابط الملف:</strong> {audioUrl || "غير متوفر"}
              </p>
              <p>
              ...
              <p>
                <strong>حالة الخطأ:</strong> {error ? "يوجد خطأ" : "طبيعي"}
              </p>
              <p>
                <strong>رسالة الخطأ:</strong>{" "}
                {errorMessage || "لا توجد أخطاء"}
              </p>
            </div>
          </details>
        </div>
