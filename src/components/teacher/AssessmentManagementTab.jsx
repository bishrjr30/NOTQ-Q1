...
import { Loader2, Play, Square, Users, BarChart3, Plus, Clock } from "lucide-react";
...
export default function AssessmentManagementTab() {
  ...
  const waitingAssessments = assessments.filter((a) => a.status === "scheduled");
  ...
  const createAssessment = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "تم حفظ التقييم كمسودة. افتحي غرفة الانتظار ثم ابدئي التقييم لاحقاً.",
      });
    ...
    <div className="space-y-6">
      ...
      <Card className="border-0 shadow-lg">
        <CardHeader className="border-b border-slate-100">
          <CardTitle className="arabic-text text-right">⏳ غرف الانتظار ({waitingAssessments.length})</CardTitle>
        </CardHeader>
        <CardContent className="p-6 space-y-4">
          {waitingAssessments.length === 0 ? (
            <p className="arabic-text text-right text-sm text-slate-500">لا توجد غرفة انتظار مفتوحة حالياً.</p>
          ) : (
            waitingAssessments.map((assessment) => {
              ...
              return (
                <Card key={assessment.id} className="border border-blue-200 bg-blue-50/40">
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="text-right">
                        ...
                        <p className="text-xs text-slate-600 arabic-text">
                          ...
                          {assessment.scheduled_start ? ` • موعد: ${toLocalDateTimeValue(assessment.scheduled_start)}` : ""}
                        </p>
                      </div>
                      <Badge className={STATUS_BADGE_CLASS.scheduled}>غرفة انتظار</Badge>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-center">
                      ...
                      <div className="bg-white rounded p-2"><p className="text-xs text-slate-500">الصف</p><p className="font-bold text-xs">{assessment.grade}</p></div>
                      <div className="bg-white rounded p-2"><p className="text-xs text-slate-500">المدة</p><p className="font-bold">{assessment.duration_minutes || 20}</p></div>
                    </div>

                    <div className="bg-white rounded-lg p-3 border border-slate-100">
                      <p className="text-xs font-bold text-slate-600 arabic-text text-right mb-2">أسماء المشاركين في الانتظار</p>
                      {participants.length === 0 ? (
                        <p className="text-xs text-slate-500 arabic-text text-right">لم يسجّل أي طالب حضوره بعد.</p>
                      ) : (
                        <div className="space-y-1">
                          {participants.map((row, i) => (
                            <div key={row.id} className="flex justify-between text-xs">
                              <span className="text-slate-500">{i + 1}</span>
                              <span className="arabic-text">{row.student_name || "طالب"}</span>
                            </div>
                          ))}
                        </div>
                      )}
        ...
        <CardContent className="p-6 space-y-3">
          {isLoading ? (
            ...
            assessments.map((assessment) => {
              ...
              const canOpenWaiting = assessment.status === "draft";
              const canStartNow = assessment.status === "scheduled";
                    ...
                    <div className="flex justify-end gap-2 mt-3">
                      {canOpenWaiting && (
                        <Button
                          size="sm"
                          variant="outline"
                          className="arabic-text border-blue-300 text-blue-700 hover:bg-blue-50"
                          onClick={() => updateAssessmentStatus(assessment, "scheduled")}
                        >
