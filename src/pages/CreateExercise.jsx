...
export default function CreateExercisePage() {
  ...
  const reviewAndCorrectText = async (originalText) => {
    try {
      setIsReviewing(true);
      const reviewPrompt = `
أنت محلّل لغوي خبير في اللغة العربية الفصحى، ومتخصص في النحو والنطق الصحيح.
مهمتك تحليل قراءة الطالب اعتمادًا على القواعد اللغوية والنطق، دون استخدام علامات رقمية أو درجات.

سيتم تزويدك بنص واحد:
- النص الذي نطق به الطالب (ناتج تحويل الصوت إلى نص)

النص:
"${originalText}"

قواعد أساسية إلزامية:
- لا تُقيِّم الترقيم إطلاقًا (الفواصل، الفاصلة المنقوطة، النقطتان، علامات الوقف).
- لا تفترض نية الطالب، وقيّم فقط ما قيل فعليًا.
- إذا كان النص فارغًا، غير مفهوم، أو لا يدل على قراءة واضحة → النتيجة "مرفوض".
- إذا كان النص مختلفًا عن المطلوب أو لا يدل على قراءة للنص الأصلي → النتيجة "مرفوض".

في حال وُجدت قراءة حقيقية للنص:
حلّل الأداء لغويًا اعتمادًا على:
1) سلامة النطق:
- نطق صحيح للحروف والكلمات
- ملاحظة الأخطاء الصوتية المؤثرة فقط
2) الالتزام بالنص:
- قراءة من النص نفسه دون إعادة صياغة
- السماح بحذف أو نقص بسيط لا يغيّر المعنى العام
3) الفهم العام:
- هل القراءة تدل على فهم للنص أم ترديد مضطرب

المطلوب منك:
- إصدار حكم تقويمي عام يعتمد على التحليل اللغوي الحقيقي
- بدون أرقام
- بدون تقسيم درجات
- بدون تفاصيل تقنية دقيقة

أعد النتيجة بصيغة JSON فقط، وبدون أي شرح خارجها:
{
  "result": "مقبول" | "بحاجة إلى تحسين" | "مرفوض",
  "feedback": "ملاحظة مهنية مختصرة توضّح سبب الحكم بشكل واضح وواقعي"
}
`;
