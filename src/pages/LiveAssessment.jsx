...
function isAssessmentOpen(assessment) {
  ...
  return assessment.status === "live" || assessment.status === "scheduled";
}

function detectQuotaError(errorLike) {
  const text = String(errorLike?.message || errorLike || "");
  return text.includes("insufficient_quota") || text.includes("429") || text.includes("exceeded your current quota");
}
...
export default function LiveAssessmentPage() {
  ...
  const joinWaitingRoom = async () => {
    try {
      ...
      toast({ title: "✅ تم تسجيل الحضور", description: "تم إدخالك إلى غرفة الانتظار بنجاح." });
      await loadAssessmentDetails(selectedAssessment.id, student.id);
    } catch (error) {
      console.error("Join waiting room failed", error);
      toast({
        title: "خطأ",
        description: "تعذر تسجيل الحضور.",
        variant: "destructive",
      });
    }
  };
  ...
  const startAttempt = async () => {
    try {
      if (selectedAssessment?.status !== "live") {
        toast({
          title: "انتظار بدء المعلم",
          description: "التقييم لم يبدأ بعد. أنتِ في غرفة الانتظار.",
          variant: "destructive",
        });
        return;
      }
  ...
  const submitCurrentAnswer = async () => {
    ...
    try {
      ...
      try {
        ...
        if (!tResponse.ok) {
          const msg = tJson?.error || `Transcribe failed with status ${tResponse.status}`;
          throw new Error(msg);
        }
        transcribedText = tJson?.text || "";
      } catch (transcribeError) {
        if (detectQuotaError(transcribeError)) {
          quotaLimited = true;
          transcribedText = "";
        } else {
...
`;
      ...
      if (quotaLimited) {
        score = 60;
        feedback = "تم حفظ إجابتك بنجاح. التقييم الذكي متوقف مؤقتاً بسبب نفاد الحصة وسيتم اعتماد تقييم تقديري لهذه الإجابة.";
        setQuotaWarning("خدمة الذكاء الاصطناعي وصلت حد الاستخدام. تم التحويل لتقييم تقديري مؤقت دون إيقاف الاختبار.");
      } else {
        try {
          ...
          score = Math.max(0, Math.min(100, Number(parsed?.score) || Math.round(matchRatio * 100)));
          feedback = parsed?.feedback || "أحسنت، واصلي التدريب.";
        } catch (llmError) {
          if (detectQuotaError(llmError)) {
            quotaLimited = true;
            score = Math.round(matchRatio * 100) || 60;
            feedback = "تم حفظ إجابتك. خدمة التحليل الذكي متوقفة مؤقتاً بسبب نفاد الحصة، وتم احتساب نتيجة تلقائية مؤقتة.";
            setQuotaWarning("تم احتساب بعض الإجابات تلقائياً بسبب انتهاء حصة API. يمكن استكمال التقييم دون توقف.");
          } else {
            throw llmError;
          }
        }
      ...
      await AssessmentSubmission.create({
        ...
        analysis_details: {
          ...
          fallback_mode: quotaLimited ? "temporary_auto_score" : null,
          word_match_ratio: Number((matchRatio * 100).toFixed(2)),
        },
                  ...
                  <CardContent className="p-6 space-y-4">
                    {selectedAssessment.status !== "live" ? (
                      <div className="space-y-3">
                        <Alert>
                          <AlertDescription className="arabic-text text-right">
                            غرفة الانتظار مفتوحة. يمكنك تسجيل الحضور الآن، وسيبدأ التقييم عند ضغط المعلم زر "بدء التقييم".
                          </AlertDescription>
                        </Alert>
                        <div className="flex justify-end gap-2">
                          <Button onClick={joinWaitingRoom} className="arabic-text bg-blue-600 hover:bg-blue-700">
                            <Users className="w-4 h-4 ml-2" />
                            تسجيل حضور
                          </Button>
                          <Button variant="outline" onClick={() => loadAssessmentDetails(selectedAssessment.id, student.id)} className="arabic-text">
                            تحديث الحالة
                          </Button>
                        </div>
                      </div>
                  ...
                  <CardContent className="p-4 space-y-2">
                    ...
                    {quotaWarning && (
                      <Alert className="border-amber-200 bg-amber-50">
                        <AlertDescription className="arabic-text text-right text-amber-900">
                          {quotaWarning}
                        </AlertDescription>
                      </Alert>
                    )}
